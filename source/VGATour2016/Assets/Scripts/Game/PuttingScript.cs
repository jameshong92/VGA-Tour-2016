...
//
//		// remove TODO
//		GlobalData.playersLeft = GlobalData.players.Count;
		BeginNewHole ();
	}
...
//		Debug.Log ("current player" + GlobalData.players[GlobalData.turn].name);
	...
	public void BeginNewHole() {
		travel.GetComponent<TravelScript> ().InitPosition ();
		GlobalData.playersLeft = GlobalData.players.Count;
		...
		// ADDED BY DIANA
		if (GlobalData.hole == 1) {
			Debug.Log ("hole created");
			GlobalData.numHoles = courses.transform.GetChild (GlobalData.course - 1).childCount;
		} else {
			Debug.Log ("not first hole created");
			foreach (GameObject g in GlobalData.balls) {
